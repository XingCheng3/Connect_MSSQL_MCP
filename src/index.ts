...
class MssqlServer {
  ...
  private setupHandlers() {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      const connArgs = args as ConnectionArgs;
      ...
      try {
        ...
        if (name === 'query' && !ENABLE_WRITE) {
          const sqlStr = (args.query as string || '').trim();
          // 简单的关键字检查，防止明显的写入操作
        ...
        switch (name) {
          ...
          case 'get_procedure_content':
            const names = args.procedureNames as string[];
  ...
  private buildConfig(args: ConnectionArgs): sql.config {
    let server = args.ip;
    const options: any = {
      encrypt: false,
    ...
    if (server.includes(',')) {
      ...
      server = parts[0];
      if (!args.port && parts[1]) args.port = parseInt(parts[1]);
    ...
    if (server === '.' || server === '(local)') server = '127.0.0.1';
